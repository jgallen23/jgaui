...
Array.prototype.remove=function(from,to){var rest=this.slice((to||from)+1||this.length);this.length=from<0?this.length+from:from;return this.push.apply(this,rest);};Array.prototype.indexOf=function(obj){for(var i=0;i<this.length;i++){if(obj==this[i])
return i;}
...
filter.push(this[i]);}
return filter;};
(function($){$.fn.textboxHint=function(hint,options){var opts=$.extend({},$.fn.textboxHint.defaults,options);var hideHint=function(){this.removeClass(opts.hintClass).val('');};var showHint=function(){this.addClass(opts.hintClass).val(hint);};var isPassword=function(){if(this.attr("type").toLowerCase()=="password")
return true;else
return false;};return this.each(function(){var $this=$(this);$this.bind("focus",function(){if($this.val()==hint)
hideHint.call($this)});$this.bind("blur",function(){if($this.val()=="")
showHint.call($this);});showHint.call($this);});};$.fn.textboxHint.defaults={hintClass:'Hint'}})(jQuery);(function($){$.fn.inlineEdit=function(options){return this.each(function(){var $this=$(this);$this.bind("click",function(){var $txt=$('<input type="text"/>');$this.hide().after($txt)
...
(function($){var templateCache={};$.fn.template=function tmpl(template,data){this.html($.template(template,data));}
