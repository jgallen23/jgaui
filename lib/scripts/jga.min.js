...
filter.push(this[i]);}
return filter;};
$.ux.behavior("PrettyDate",{initialize:function(){var prettyDate=this.getPrettyDate(this.element.html());this.element.html(prettyDate);},getPastPrettyDate:function(day_diff,diff){return day_diff==0&&(diff<60&&"just now"||diff<120&&"1 minute ago"||diff<3600&&Math.floor(diff/60)+" minutes ago"||diff<7200&&"1 hour ago"||diff<86400&&Math.floor(diff/3600)+" hours ago")||day_diff==1&&"Yesterday"||day_diff<7&&day_diff+" days ago"||false},getFuturePrettyDate:function(day_diff,diff){return day_diff==0&&(diff<60&&"in a minute"||diff<360&&"in a few minutes"||diff<3600&&"in "+Math.floor(diff/60)+" minutes"||diff<7200&&"in 1 hour"||diff<86400&&"in "+Math.floor(diff/3600)+" hours")||day_diff==1&&"Tomorrow"||day_diff<7&&"in "+day_diff+" days"||false},getPrettyDate:function(time){var date=new Date((time||"").replace(/-/g,"/").replace(/[TZ]/g," ")),diff=(((new Date()).getTime()-date.getTime())/1000),day_diff=Math.floor(Math.abs(diff)/86400);console.log(time,day_diff,diff);if(isNaN(day_diff))
return time;else if(diff>0)
return this.getPastPrettyDate(day_diff,diff)||time;else if(diff<0)
return this.getFuturePrettyDate(day_diff,-diff)||time;}});(function($){$.prettyDate=function(time){var getPastPrettyDate=function(day_diff,diff){return day_diff==0&&(diff<60&&"just now"||diff<120&&"1 minute ago"||diff<3600&&Math.floor(diff/60)+" minutes ago"||diff<7200&&"1 hour ago"||diff<86400&&Math.floor(diff/3600)+" hours ago")||day_diff==1&&"Yesterday"||day_diff<7&&day_diff+" days ago"||false};var getFuturePrettyDate=function(day_diff,diff){return day_diff==0&&(diff<60&&"in a minute"||diff<360&&"in a few minutes"||diff<3600&&"in "+Math.floor(diff/60)+" minutes"||diff<7200&&"in 1 hour"||diff<86400&&"in "+Math.floor(diff/3600)+" hours")||day_diff==1&&"Tomorrow"||day_diff<7&&"in "+day_diff+" days"||false};var failOver=function(time){if(time.indexOf("T")==-1){var d=new Date(parseInt(time));return(d.getMonth()+1)+"/"+d.getDate()+"/"+d.getFullYear();}else
...
(function($){$.fn.textboxHint=function(hint,options){var opts=$.extend({},$.fn.textboxHint.defaults,options);var hideHint=function(){this.removeClass(opts.hintClass).val('');};var showHint=function(){this.addClass(opts.hintClass).val(hint);};return this.each(function(){var $this=$(this);$this.bind("focus",function(){if($this.val()==hint)
hideHint.call($this)});$this.bind("blur",function(){if($this.val()=="")
showHint.call($this);});if($this.val()=="")
showHint.call($this);});};$.fn.textboxHint.defaults={hintClass:'Hint'}})(jQuery);
(function($){$.ux.behavior('Slider',{initialize:function(){this._totalItems=$("li",this.element).length;this._totalPages=Math.ceil(this._totalItems/this.options.numberPerPage);this._currentPage=1;this.prevButton=$(".prev",this.element).hide();this.nextButton=$(".next",this.element);},onclick:$.delegate({'.prev':function(element,event){if(this._currentPage>1)
this._currentPage--;if(this._currentPage==1){element.hide();}else{element.show();}
this.nextButton.show();this.slide();},'.next':function(element,event){if(this._currentPage<this._totalPages)
this._currentPage++;if(this._totalPages==(this._currentPage)){element.hide();}else{element.show();}
this.prevButton.show();this.slide();}}),slide:function(){$("ul",this.element).animate({"left":-(this._currentPage-1)*this.options.elementWidth});}},{elementWidth:100,numberPerPage:1});})(jQuery);